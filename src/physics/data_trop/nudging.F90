...
!=====================================================================
  ...
  integer :: pverob = 0                        ! vertical levels in obs file (set from obs file)
...
!  real(r8),allocatable::pmidob(:,:,:,:)     !(pcols,pverob,begchunk:endchunk,Nudge_NumObs)   obs mid-level pressures
  ...
  real(r8),allocatable::Nobs_Q_remap(:,:,:,:) !(pcols,pver,begchunk:endchunk,Nudge_NumObs)
...
contains
  ...
  subroutine nudging_init
   ...
   ! Read pverob (obs vertical level count) from the first obs file.
   !-----------------------------------------------------------------
   ...
   call cam_pio_openfile(fileID_tmp, trim(Nudge_Path)//trim(Nudge_File), 0)
   ierr = pio_inq_dimid(fileID_tmp, 'lev', dim_id)
   if (ierr /= 0) call endrun(prefix//'FATAL: pio_inq_dimid: lev')
   ierr = pio_inq_dimlen(fileID_tmp, dim_id, pverob)
   if (ierr /= 0) call endrun(prefix//'FATAL: pio_inq_dimlen: lev')
   call pio_closefile(fileID_tmp)
   if(masterproc) write(iulog,*) 'NUDGING: nudging_init() pverob=',pverob,' (read from obs file)'
   
   ! Allocate Space for Nudging observation arrays, initialize with 0's
   !---------------------------------------------------------------------
   allocate(Nobs_U(pcols,pverob,begchunk:endchunk,Nudge_NumObs),stat=istat)
   call alloc_err(istat,'nudging_init','Nobs_U',pcols*pver*((endchunk-begchunk)+1)*Nudge_NumObs)
   allocate(Nobs_V(pcols,pverob,begchunk:endchunk,Nudge_NumObs),stat=istat)
   call alloc_err(istat,'nudging_init','Nobs_V',pcols*pver*((endchunk-begchunk)+1)*Nudge_NumObs)
   allocate(Nobs_T(pcols,pverob,begchunk:endchunk,Nudge_NumObs),stat=istat)
   call alloc_err(istat,'nudging_init','Nobs_T',pcols*pver*((endchunk-begchunk)+1)*Nudge_NumObs)
   allocate(Nobs_Q(pcols,pverob,begchunk:endchunk,Nudge_NumObs),stat=istat)
   ...
   allocate(Nobs_V_remap(pcols,pver,begchunk:endchunk,Nudge_NumObs),stat=istat)
   call alloc_err(istat,'nudging_init','Nobs_V_remap',pcols*pver*((endchunk-begchunk)+1)*Nudge_NumObs)
   allocate(Nobs_T_remap(pcols,pver,begchunk:endchunk,Nudge_NumObs),stat=istat)
   call alloc_err(istat,'nudging_init','Nobs_T_remap',pcols*pver*((endchunk-begchunk)+1)*Nudge_NumObs)
   allocate(Nobs_Q_remap(pcols,pver,begchunk:endchunk,Nudge_NumObs),stat=istat)
   call alloc_err(istat,'nudging_init','Nobs_Q_remap',pcols*pver*((endchunk-begchunk)+1)*Nudge_NumObs)
   
   Nobs_U(:pcols,:pverob,begchunk:endchunk,:Nudge_NumObs)=0._r8
   ...
   Nobs_T_remap(:pcols,:pver,begchunk:endchunk,:Nudge_NumObs)=0._r8
   Nobs_Q_remap(:pcols,:pver,begchunk:endchunk,:Nudge_NumObs)=0._r8
...
!+++arh            
   ...
   if((Before_End) .and. ((Update_Nudge) .or. (Update_Model))) then
     ...
     if(Nudge_Force_Opt == 0) then
       ...
       do lchnk=begchunk,endchunk
         ncol=phys_state(lchnk)%ncol
         Target_U(:ncol,:pver,lchnk)=Nobs_U_remap(:ncol,:pver,lchnk,Nudge_ObsInd(1))
         Target_V(:ncol,:pver,lchnk)=Nobs_V_remap(:ncol,:pver,lchnk,Nudge_ObsInd(1))
     ...
     elseif(Nudge_Force_Opt == 1) then
       ...
       do lchnk=begchunk,endchunk
         ncol=phys_state(lchnk)%ncol
         Target_U(:ncol,:pver,lchnk)=(1._r8-Tfrac)*Nobs_U_remap(:ncol,:pver,lchnk,Nudge_ObsInd(1)) &
                                           +Tfrac *Nobs_U_remap(:ncol,:pver,lchnk,Nudge_ObsInd(2))
         Target_V(:ncol,:pver,lchnk)=(1._r8-Tfrac)*Nobs_V_remap(:ncol,:pver,lchnk,Nudge_ObsInd(1)) &
                                           +Tfrac *Nobs_V_remap(:ncol,:pver,lchnk,Nudge_ObsInd(2))
         Target_T(:ncol,:pver,lchnk)=(1._r8-Tfrac)*Nobs_T_remap(:ncol,:pver,lchnk,Nudge_ObsInd(1)) &
...
!    endif
  ...
  subroutine nudging_update_analyses(anal_file)
   ...
   use cam_pio_utils   ,only: cam_pio_openfile
   use pio             ,only: PIO_BCAST_ERROR,PIO_INTERNAL_ERROR,PIO_NOERR
   use pio             ,only: pio_closefile,pio_seterrorhandling,file_desc_t
   use pio             ,only: pio_inq_varid,pio_get_var,var_desc_t
   ...
   allocate(Tmp3D(pcols,pverob,begchunk:endchunk))
   ...
   if (ierr /= PIO_NOERR) call endrun('Variable "hyai" is missing in '//trim(anal_file))
   ierr = pio_get_var(fileID, varid, hyaiob)
   if (ierr /= PIO_NOERR) call endrun('Failed to read "hyai" from '//trim(anal_file))

   ierr = pio_inq_varid(fileID, 'hybi', varid)
   if (ierr /= PIO_NOERR) call endrun('Variable "hybi" is missing in '//trim(anal_file))
   ierr = pio_get_var(fileID, varid, hybiob)
   if (ierr /= PIO_NOERR) call endrun('Failed to read "hybi" from '//trim(anal_file))
...
!     endif
   ...
   call infld('PINT',fileID,dim1name,'ilev',dim2name,     &
              1,pcols,1,pverob+1,begchunk,endchunk,Tmp3Dp, &
              VARflag,gridname='physgrid',timelevel=1 )
   if(VARflag) then
!     if(Nudge_ZonalFilter) then
!       call ZM%calc_amps(Tmp3Dp,Zonal_Bamp3d)
!       call ZM%eval_grid(Zonal_Bamp3d,Tmp3Dp)
!     endif
     pintob(:,:,begchunk:endchunk,Nudge_ObsInd(1)) = Tmp3Dp(:,:,begchunk:endchunk)
   else
     call endrun('Variable "PINT" is missing in '//trim(anal_file))
   endif
...
!   end do
  ...
  subroutine nudging_final
    ...
    if (allocated(Nobs_Q_remap)) deallocate(Nobs_Q_remap)
